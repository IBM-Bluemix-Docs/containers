...
**Attention**: Updating the worker node version can cause downtime for your apps and services. If worker nodes are available, all pods are rescheduled onto other worker nodes, and data is deleted if not stored outside the pod. 
...
3. View resource usage of the worker nodes. During an update, worker nodes are removed sequentially, and pods are rescheduled if worker nodes are available. If you need more space for your apps, add worker nodes to your cluster.
...
8. If the app does not work, delete the app and adjust your deployment script before trying again.
