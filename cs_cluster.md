...
**Attention**: Updating the worker node version can cause downtime for your apps and services. During the update, all pods are rescheduled onto other worker nodes, and data is deleted if not stored outside the pod. 

1. Back up all data prior to updating worker nodes.
2. Complete the steps for the [Kubernetes changes](cs_versions.html), where you ensure that future app deployments continue to run. 
3. Confirm that you have enough capacity for your apps. During an update, one worker node is removed at a time, and pods are rescheduled to other worker nodes. If you need more space, add worker nodes to your cluster.
...
6. Make any required changes to your deployment scripts and redeploy. Because the updated worker node has no workload, your pods will deploy to that worker node.
7. Monitor your app to confirm that it is working on the updated worker node.
8. If the app does not work, [drain ![External link icon](../icons/launch-glyph.svg "External link icon")](https://kubernetes-v1-4.github.io/docs/user-guide/kubectl/kubectl_drain/) the worker node and adjust your deployment script before trying again.
9. When the deployment works as expected, update your remaining worker nodes.
