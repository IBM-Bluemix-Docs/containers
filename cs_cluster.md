...
4.  Specify the {{site.data.keyword.Bluemix_notm}} organization and space where you want to create your Kubernetes cluster.
    ...
    If you want to specify a different organization or space than the one that `bx target --cf` targeted, you can retrieve a list of the organizations or spaces available and then target the ones you want.
    ```
    bx iam orgs
    bx iam spaces
    ...
    **Note**: Clusters are specific to an account and an organization, but are independent from a {{site.data.keyword.Bluemix_notm}} space. For example, if you create a cluster in your organization in `test` space, you can still work with that cluster if you later target the same organization but `dev` space.
