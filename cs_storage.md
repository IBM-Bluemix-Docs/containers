...
3.  Get the details for a storage class. Review the IOPS per gigabyte and the size range in the **paramters** field in your CLI output. 
    ...
    -  With custom storage classes, you get [Performance storage ![External link icon](../icons/launch-glyph.svg "External link icon")](https://knowledgelayer.softlayer.com/topic/performance-storage) and have more control over choosing the combination of IOPS and size. **Example command to describe custom storage class**:
