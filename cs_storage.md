...
File shares are provisioned into the same location (data center) as your cluster and are clustered by {{site.data.keyword.IBM_notm}} to provide high availability. However, file shares are not backed up automatically and might be inaccessible if the entire location fails. To protect your data from being lost or damaged, you can set up periodic backups that you can use to restore your data when needed.
{: shortdesc}
...
<dl>
  ...
  <dd><p><strong>Set up periodic snapshots</strong>You can set up [periodic snapshots](/docs/infrastructure/FileStorage/snapshots.html) for your NFS file share, which is a read-only image that captures the state of the volume at a point in time. Snapshots are stored on the same file share within the same location. You can restore data from a snapshot if a user accidentally removes important data from the volume.</p>
 ...
 <dt>Create duplicate storage</dt>
 <dd><p>You can create a duplicate of your NFS file share volume in the same location (data center) as the original volume. To duplicate, first set up snapshots for the volume. A duplicate has the same data as the original volume at the point in time that you create the duplicate. Unlike replicas, use the duplicate as a completely independent volume from the original.</p>
...
<dt>Copy files and directories to and from containers</dt>
<dd><p>You can use the `kubectl cp` command to copy files and directories to and from containers. Before you begin, [target your Kubernetes CLI](cs_cli_install.html#cs_cli_configure) to the cluster that you want to use. <strong>Note</strong>: The copy command requires that the <code>tar</code> binary is present within the container image.</p>
<p>You can use the command in a variety of ways:</p>
<ul><li>Copy from a local source to a local destination: <code>kubectl cp <var>filepath/src</var> <var>filepath/dest</var></code></li>
<li>Copy from a local source to a remote pod: <code>kubectl cp <var>filepath/src</var> <var>namespace/pod:filepath/dest</var></code></li>
<li>Copy from a remote pod to a local destination: <code>kubectl cp <var>namespace/pod:filepath/src</var> <var>filepath/dest</var></code></li>
<li>Copy from a remote pod to another pod in a specific container: <code>kubectl cp <var>namespace/pod:filepath/src</var> <var>namespace/pod:filepath/dest</var> -c <var>container</var></code></li>
</ul></dd>
  </dl>
