...
5. The VPN tunnel endpoint (router) forwards the request to the on-premises server or mainframe depending on the destination IP address specified by the application in step 1. The necessary data is sent back over the VPN connection to `myapp` through the same process.
...
The [Vyatta Gateway Appliance [![External link icon](../icons/launch-glyph.svg "External link icon")]](http://knowledgelayer.softlayer.com/learning/network-gateway-devices-vyatta) is a bare metal server that runs a special distribution of Linux. You can use a Vyatta as VPN gateway to connect to an on-premises network.
{:shortdesc}
...
The following diagram shows how a Vyatta secures a connection between the worker nodes and an on-premises data center:
...
2. Because `myapp2` is on a worker node that is on a private VLAN only, the Vyatta acts as a secure connection between the worker nodes and the on-premises network. The Vyatta forwards the request to the on-premises data center.

3. The request is encrypted and sent over the VPN tunnel to the on-premises data center. The destination IP address is used to determine which network packets should be sent to the on-premises network.

4. The incoming request passes through the on-premises firewall and is delivered to the VPN tunnel endpoint (router) where it is decrypted.

5. The VPN tunnel endpoint (router) forwards the request to the on-premises server or mainframe depending on the destination IP address specified by `myapp2` in step 3. The necessary data is sent back over the VPN connection to `myapp2` through the same process.
